...
public class AssessmentPart1 {
	...
	*/
	public int Q04_moviePriceMatinee(int age, boolean isBefore5pm, boolean isWeekend) {
		int adultPrice = 12;
		int childPrice = 7;
		int adultMatineePrice = 8;
		int childMatineePrice = 5;
		if (isBefore5pm && !isWeekend) {
			if (age <= 12) {
				return childMatineePrice;
			} else {
				return adultMatineePrice;
			}
		} else {
			if (age <= 12) {
				return childPrice;
...
//		return stringValue;
		String result = stringValue.replace(" ", replaceSpaceWith);
		return result;
	}


	/*
	Topics: Datatypes, Expressions

	Fix the bug!
	The purpose of this method is to calculate the Fahrenheit temperature value from a known Celsius temperature.
	To calculate `degreesFahrenheit`, multiple nine-fifths (9/5) times `degreeCelsius`, and to that result, add 32.
	Tests are failing, and you have been asked to fix the bug in this method. For example, when this method is called
...
//		double degreesCelsius = degreesFahrenheit - 32.0 * 5.0 / 9.0;
	...
	*/
	public int[] Q10_swapFirstAndLastElements(int[] arrayOfInts) {
		...
		arrayOfInts[arrayOfInts.length-1] = firstValue;
		return arrayOfInts;
	}
	...
	*/
	public Map<String, Integer> Q11_mealCount(String[] mealOrders) {
		...
		for(String meal : mealOrders) {
			if (cateringOrder.containsKey(meal)) {
				cateringOrder.replace(meal, cateringOrder.get(meal) + 1);
			} else {
				cateringOrder.put(meal, 1);
			}
		}
