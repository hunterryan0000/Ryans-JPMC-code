...
public class JdbcLineItemDaoTests extends BaseDaoTests {
    ...
    @Test
    public void getLineItemsBySale_returns_correct_items_for_sale() {
        List<LineItem> lineItems = lineItemDao.getLineItemsBySale(1);
        Assert.assertEquals("getLineItemsBySale returned wrong number of sales", 3, lineItems.size());
        Assert.assertEquals("getLineItemsBySale: first line item id does not match expected, check sort order",
                LI_1.getLineItemId(), lineItems.get(0).getLineItemId());
        assertLineItemsMatch("getLineItemsBySale returned wrong or partial data", LI_1, lineItems.get(0));
        assertLineItemsMatch("getLineItemsBySale returned wrong or partial data", LI_2, lineItems.get(1));
        assertLineItemsMatch("getLineItemsBySale returned wrong or partial data", LI_3, lineItems.get(2));

        lineItems = lineItemDao.getLineItemsBySale(2);
        Assert.assertEquals("getLineItemsBySale returned wrong number of sales", 2, lineItems.size());
        Assert.assertEquals("getLineItemsBySale: first line item id does not match expected, check sort order",
                LI_4.getLineItemId(), lineItems.get(0).getLineItemId());
        assertLineItemsMatch("getLineItemsBySale returned wrong or partial data", LI_4, lineItems.get(0));
        assertLineItemsMatch("getLineItemsBySale returned wrong or partial data", LI_5, lineItems.get(1));

        lineItems = lineItemDao.getLineItemsBySale(3);
        Assert.assertEquals("getLineItemsBySale returned wrong number of sales", 1, lineItems.size());
        assertLineItemsMatch("getLineItemsBySale returned wrong or partial data", LI_6, lineItems.get(0));

        lineItems = lineItemDao.getLineItemsBySale(99);
        Assert.assertEquals("getLineItemsBySale returned wrong number of sales", 0, lineItems.size());
