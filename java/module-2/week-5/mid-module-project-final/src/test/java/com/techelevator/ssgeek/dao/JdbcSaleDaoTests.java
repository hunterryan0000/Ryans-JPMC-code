...
public class JdbcSaleDaoTests extends BaseDaoTests {
    ...
    public void getSale_returns_correct_sale_for_id() {
        ...
        Assert.assertNotNull("getSale returned null", sale);
        assertSalesMatch("getSale returned wrong or partial data", SALE_4, sale);
    }

    @Test
    public void getSale_returns_null_when_id_not_found() {
        Sale sale = saleDao.getSale(99);
        Assert.assertNull("getSale failed to return null for id not in database", sale);
    }

    @Test
    public void getSalesUnshipped_returns_sales_with_null_ship_date() {
        List<Sale> sales = saleDao.getSalesUnshipped();
        Assert.assertEquals("getSalesUnshipped returned wrong number of sales", 2, sales.size());
        Assert.assertEquals("getSalesUnshipped: first sale id does not match expected, check sort order",
                SALE_1.getSaleId(), sales.get(0).getSaleId());
        assertSalesMatch("getSalesUnshipped returned wrong or partial data", SALE_1, sales.get(0));
        assertSalesMatch("getSalesUnshipped returned wrong or partial data", SALE_3, sales.get(1));
    }
